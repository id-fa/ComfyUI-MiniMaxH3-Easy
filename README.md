...
Click `✦` at the bottom-right of the prompt editor to optimize the current prompt manually, or enable automatic optimization when the workflow runs. Supported providers include:
...
API settings are stored in `prompt_optimizer.json` inside the plugin directory. API keys are stored as plain text in that file, so do not publish or upload it.
